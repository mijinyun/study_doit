...
function nameValue(x? :string) :void{
...
function valueCheck(x: string | number) : number {
...
function marry(income :number, house :boolean, attr :string) : string|void {
    let score :number =0;
    score += income;

    if(house) score += 500;
    if(attr == '상') score += 100;
    if(income >= 600) console.log('결혼가능');
